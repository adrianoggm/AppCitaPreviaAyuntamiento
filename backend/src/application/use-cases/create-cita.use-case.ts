...
*/
  async execute(createCitaDto: CreateCitaDto): Promise<Cita> {
    ...
    const departamentos =
      await this.departamentoRepository.findDepartamentoByTipoTramite(
        tipoTramite
      );
    if (!departamentos || departamentos.length === 0) {
      throw new NotFoundException(
        'No se encontraron departamentos para el tipo de trámite indicado.'
      );
    }
    ...
    if (localizacion) {
      tecnicos =
        await this.tecnicoRepository.findByDepartamentosAndLocalizacion(
          codigosDepartamento,
          localizacion
        );
    } else {
      tecnicos =
        await this.tecnicoRepository.findByDepartamentos(codigosDepartamento);
    }
    if (!tecnicos || tecnicos.length === 0) {
      throw new NotFoundException(
        'No hay técnicos disponibles para el departamento o localización indicada.'
      );
    }
    ...
    const citas = await this.citaRepository.findBetweenDatesForTecnicos(
      fechaini,
      fechafin,
      tecnicos.map((t) => t.id)
    );
    ...
    const tecnicosDisponiblesIds = tecnicos
    ...
    if (tecnicosDisponiblesIds.length === 0) {
      throw new ConflictException(
        'No hay técnicos disponibles para esa franja horaria.'
      );
    }

    //  Se coge el primer tecnico encontrado
    const idtecnicoAsignado = tecnicosDisponiblesIds[0];
    const tecnicoAsignado = tecnicos.find(
      (t) => t.id.toString() === idtecnicoAsignado
    );
    if (!tecnicoAsignado) {
    ...
    const citaData = {
      ...
      departamento: tecnicoAsignado.departamento,
    };
