...
export class ObtenerDiasMesCitaUseCase {
  ...
  async execute(
    ...
    if (!tecnicos || tecnicos.length === 0) {
      throw new NotFoundException(
        'No hay técnicos disponibles para la localización o departamento indicado.'
      );
    }
      ...
      for (let hora = 8; hora < 14 && !tieneDisponibilidad; hora++) {
        for (let minute of [0, 30]) {
          ...
          const tecnicosOcupados = citas
            .filter(
              (cita) => new Date(cita.fechahora).toISOString() === slotISO
            )
